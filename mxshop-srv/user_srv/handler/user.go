...
func (dl *DemoListener) ExecuteLocalTransaction(msg *primitive.Message) primitive.LocalTransactionState {
	...
	user = model.User{Nickname: msgs["nickname"].(string), Password: library.MD5(msgs["password"].(string)), Mobile: msgs["mobile"].(string)}
	...
	//注册送积分
	_, err = global.CouponsClient.SendCouponsToUser(context.Background(), &proto.SendCouponsToUserRequest{
		Mobile:    mobile,
...
func (u *UserService) CreateUser(ctx context.Context, info *proto.CreateUserInfoRequest) (*proto.UserInfoResponse, error) {
	...
	if localExecDict[info.Mobile]["code"].(codes.Code) == codes.OK {
		return localExecDict[info.Mobile]["resp"].(*proto.UserInfoResponse), nil
	}
